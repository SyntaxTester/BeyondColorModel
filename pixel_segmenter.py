...
# Keep the deploy palette as the first-choice palette for up to eight data
# series.  Its patterns were selected from a measured pairwise-distance matrix,
# not simply cycled, so a small chart gets the most distinguishable combination
# available.  The newer library below remains the extension for charts with
# more than eight series.
_DEPLOY_PATTERN_FAMILIES = [
    "dots", "herringbone", "triangles", "lines", "rings", "star", "plus",
    "checker",
]
_DEPLOY_PATTERN_FILL = {
    "dots": 0.2193,
    "herringbone": 0.1875,
    "triangles": 0.3594,
    "lines": 0.2969,
...
_DEPLOY_SHAPE_FAMILIES = {"star", "triangles", "rings", "plus"}
_DEPLOY_ANGLES = [0.0, 90.0, 22.5, 112.5, 45.0, 135.0, 67.5, 157.5]
_DEPLOY_PATTERN_DISTANCE = [
...
def _pick_deploy_families(count: int) -> list[int]:
    """Return the deploy palette subset with the largest minimum distance."""
    import itertools

    total = len(_DEPLOY_PATTERN_FAMILIES)
    if count <= 0:
        return []
    if count >= total:
    ...
    for selection in itertools.combinations(range(total), count):
        minimum_distance = min(
            _DEPLOY_PATTERN_DISTANCE[left][right]
            for left, right in itertools.combinations(selection, 2)
        )
        # Keep the first combination on ties, just as the deploy version did:
        # this makes the palette selection stable across runs and Python versions.
        if best is None or minimum_distance > best[0]:
            best = (minimum_distance, selection)
    assert best is not None
    return list(best[1])


def _deploy_pattern_spec(index: int) -> _PatternSpec:
    family = _DEPLOY_PATTERN_FAMILIES[index]
    angle = 0.0 if family in _DEPLOY_SHAPE_FAMILIES else _DEPLOY_ANGLES[index]
    return _PatternSpec(family, angle)


def _pattern_specs_for_series(count: int) -> list[_PatternSpec]:
    """Combine deploy's max-min palette with the newer extended library."""
    deploy_count = min(count, len(_DEPLOY_PATTERN_FAMILIES))
    specs = [_deploy_pattern_spec(index) for index in _pick_deploy_families(deploy_count)]
    for series_id in range(count - deploy_count):
        specs.append(_pattern_spec_for_series(series_id))
    return specs
...
def _merge_duplicate_legend_swatch_handles(handles: list[dict]) -> list[dict]:
    """Merge colour samples that resolve to the same physical legend swatch.

    A rasterised line can yield a core colour and anti-aliased edge colours.
    They all locate the same legend key, but must remain one logical series.
    """
    merged: list[list[dict]] = []
    for handle in sorted(handles, key=lambda item: (item["y"], item["x"])):
        left = int(handle["x"])
        right = left + int(handle["len"])
        for group in merged:
            reference = group[0]
            ref_left = int(reference["x"])
            ref_right = ref_left + int(reference["len"])
            vertical_tolerance = max(2, (int(handle["h"]) + int(reference["h"])) // 2)
            overlaps = max(left, ref_left) <= min(right, ref_right) + 2
            if abs(int(handle["y"]) - int(reference["y"])) <= vertical_tolerance and overlaps:
                group.append(handle)
                break
        else:
    ...
    return [
        {
            "x": min(int(item["x"]) for item in group),
            "len": max(int(item["x"]) + int(item["len"]) for item in group)
                   - min(int(item["x"]) for item in group),
            "h": max(int(item["h"]) for item in group),
            "y": round(sum(int(item["y"]) for item in group) / len(group)),
            "rgb": tuple(np.rint(np.mean([item["rgb"] for item in group], axis=0)).astype(int)),
        }
        for group in merged
    ]
...
def _assign_patterns_by_color(series: list[_PixelSeries]) -> None:
    ...
    ordered_clusters = sorted(clusters, key=lambda item: _hue_sort_key(item["mean_rgb"]))
    for series_id, (cluster, spec) in enumerate(
        zip(ordered_clusters, _pattern_specs_for_series(len(ordered_clusters)))
    ):
...
def _find_peaks(
    ...
    """Find well-separated local extrema for the deploy line-marker priority."""
    if len(cx_all) < 40:
        return []
    order = np.argsort(cx_all)
    xs_sorted = cx_all[order]
    ys_sorted = cy_all[order]
    unique_x = np.unique(xs_sorted)
    if len(unique_x) < 20:
        return []

    median_y = np.zeros(len(unique_x), dtype=np.float64)
    start = 0
    for index, x_value in enumerate(unique_x):
        end = start
        while end < len(xs_sorted) and xs_sorted[end] == x_value:
            end += 1
        median_y[index] = np.median(ys_sorted[start:end])
        start = end

    smoothed = np.convolve(median_y, np.ones(smooth) / smooth, mode="same")
    derivative = np.diff(smoothed)
    candidates: list[int] = []
    for index in range(2, len(derivative) - 2):
        if derivative[index - 1] * derivative[index] >= 0:
            continue
        left = max(0, index - 15)
        right = min(len(smoothed), index + 15)
        if abs(smoothed[index] - smoothed[left:right].mean()) >= prominence:
            candidates.append(int(unique_x[index]))

    peaks: list[int] = []
    for x_value in candidates:
        if not peaks or x_value - peaks[-1] > min_gap:
            peaks.append(x_value)
    return peaks
...
def _place_markers_on_lines(
    ...
    if len(inferred_handles) == len(clustered_colors):
        handles = _merge_duplicate_legend_swatch_handles(inferred_handles)
    elif legend_handles and len(legend_handles) >= len(clustered_colors):
        handles = _merge_duplicate_legend_swatch_handles(legend_handles)
    else:
        handles = []
...
def _draw_pattern(
    ...
    if family in _DEPLOY_PATTERN_FILL:
        fill = _DEPLOY_PATTERN_FILL[family]
        base_spacing = spacing * 0.72 if bool(np.asarray(bold).ravel()[0]) else spacing
        deploy_spacing = float(max(4, int(round(base_spacing))))

        def pixel_size(value: float) -> float:
            return float(max(1, int(round(value))))

        if family == "dots":
            gx = proj % deploy_spacing
            gy = perp % deploy_spacing
            center = deploy_spacing / 2
            radius = pixel_size(deploy_spacing * fill)
            return (gx - center) ** 2 + (gy - center) ** 2 <= radius ** 2

        if family == "herringbone":
            row = np.floor(perp / (deploy_spacing * 0.9))
            diagonal = np.where((row % 2) == 1, proj + perp, proj - perp)
            return (diagonal % deploy_spacing) < pixel_size(deploy_spacing * fill)

        if family == "triangles":
            u = (proj % deploy_spacing) / deploy_spacing
            v = (perp % deploy_spacing) / deploy_spacing
            centered_u = u - 0.5
            lower, upper = 0.5 - fill, 0.5 + fill
            progress = np.clip((v - lower) / (upper - lower), 0.0, 1.0)
            return (v >= lower) & (v <= upper) & (np.abs(centered_u) <= fill * progress)

        if family == "lines":
            return (proj % deploy_spacing) < pixel_size(deploy_spacing * fill)

        if family == "rings":
            gx = (proj % (deploy_spacing * 1.7)) - deploy_spacing * 0.85
            gy = (perp % (deploy_spacing * 1.7)) - deploy_spacing * 0.85
            distance = np.sqrt(gx ** 2 + gy ** 2)
            return np.abs(distance - pixel_size(deploy_spacing * 0.62)) < pixel_size(deploy_spacing * fill)

        if family == "plus":
            row = np.floor(perp / deploy_spacing)
            offset = (row % 2) * deploy_spacing * 0.5
            u = ((proj + offset) % deploy_spacing) - deploy_spacing * 0.5
            v = (perp % deploy_spacing) - deploy_spacing * 0.5
            arm = pixel_size(deploy_spacing * fill * 0.60)
            thickness = pixel_size(deploy_spacing * fill * 0.20)
            return ((np.abs(u) < thickness) & (np.abs(v) < arm)) | \
                   ((np.abs(v) < thickness) & (np.abs(u) < arm))

        if family == "star":
            u = ((proj % deploy_spacing) / deploy_spacing) - 0.5
            v = ((perp % deploy_spacing) / deploy_spacing) - 0.5
            radius = np.sqrt(u ** 2 + v ** 2) + 1e-6
            angle = np.arctan2(v, u)
            lobe = 0.5 + 0.5 * np.cos(angle * 5.0)
            return radius < (fill * (0.5 + lobe))

        if family == "checker":
            columns = np.floor(proj / deploy_spacing).astype(np.int64)
            rows = np.floor(perp / deploy_spacing).astype(np.int64)
            return ((columns + rows) % 2) == 0
...
def classify_chart_kind(
    image: Image.Image,
    smooth_shadows: bool = True,
    detect_lines: bool = True,
) -> str:
    """Classify a chart without drawing on it.

    The SAM pipeline uses this to choose the pie-specific segmentation rules
    instead of relying on a layout detector's tentative figure label.
    """
    import cv2
    ...
    channel_min = arr_for_classification.min(axis=2)
    delta = channel_max.astype(np.float32) - channel_min.astype(np.float32)
    colorful = (delta >= 25) & (saturation >= 0.12)
    line_valid = (
        (delta >= 25)
        & (channel_max >= 30)
        & (channel_min <= 248)
    ...
    return _classify_chart_geometry(
        arr,
        line_valid,
        colorful,
        protected,
